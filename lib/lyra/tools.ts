...
- git_push: Push to remote`,
  ...
  { name: "local_business_search", description: "Search local businesses, get reviews and details. action: search|details|reviews|nearby. query=keyword, location=city, business_id, lat, lng, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, location: {type:"string"}, business_id: {type:"string"}, lat: {type:"string"}, lng: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "job_search", description: "Search real job listings from LinkedIn, Indeed, Glassdoor. action: search|details|estimated_salary. query=job title, location, job_id, employment_type, remote, date_posted, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, location: {type:"string"}, job_id: {type:"string"}, employment_type: {type:"string"}, remote: {type:"string"}, date_posted: {type:"string"}, limit: {type:"number"} }, required: ["action","query"] } },
  { name: "youtube_search", description: "Search YouTube videos, trending, comments. action: search|video|trending|comments. query, video_id, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, video_id: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "news_search", description: "Search real-time news articles. action: search|top. query=topic, category=technology|business|sports|etc, country=US, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, category: {type:"string"}, country: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "reddit_search", description: "Search Reddit posts and subreddits. action: search|subreddit|hot. query, subreddit=name without r/, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, subreddit: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "currency_convert", description: "Convert currencies and get live exchange rates. action: convert|rates|list. from=USD, to=EUR, amount=1.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, from: {type:"string"}, to: {type:"string"}, amount: {type:"number"} }, required: ["action"] } },
  { name: "movie_search", description: "Search movies and TV shows from IMDb. action: search|details|trending. query, imdb_id, type=movie|tvSeries, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, imdb_id: {type:"string"}, type: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "amazon_search", description: "Search Amazon products, prices, reviews. action: search|details. query, asin, country=US, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, asin: {type:"string"}, country: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "spotify_search", description: "Search Spotify for music. action: search|track|artist. query, id=spotify ID, type=track|artist|album, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, id: {type:"string"}, type: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "instagram_search", description: "Get public Instagram profiles, posts, hashtags. action: profile|posts|hashtag. username or hashtag.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, username: {type:"string"}, hashtag: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "phone_forensics", description: "Phone forensics — carrier, owner, spam score, line type. RESTRICTED paid clients and admins only. action: lookup|validate|reputation|owner. phone=number, country=US.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, phone: {type:"string"}, country: {type:"string"} }, required: ["action","phone"] } },
  { name: "site_audit", description: "Full website audit — PageSpeed, SSL grade, security headers, WHOIS, tech stack. action: full|performance|ssl|headers|whois|tech. url=website URL.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, url: {type:"string"} }, required: ["action","url"] } },
  { name: "security_scan", description: "Security scan — DNS security SPF/DMARC/DKIM, HTTP headers, open ports. action: dns_security|headers|ports|full. target=domain.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, target: {type:"string"} }, required: ["action","target"] } },
  { name: "cve_lookup", description: "Look up CVE vulnerabilities from NVD/NIST. action: search|details. query=software name, cve_id=CVE-XXXX-XXXXX, limit.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, query: {type:"string"}, cve_id: {type:"string"}, limit: {type:"number"} }, required: ["action"] } },
  { name: "virustotal_scan", description: "Scan URLs, domains, IPs for malware via VirusTotal. action: url|domain|ip. target=URL or domain or IP.", input_schema: { type: "object" as const, properties: { action: {type:"string"}, target: {type:"string"} }, required: ["action","target"] } },
