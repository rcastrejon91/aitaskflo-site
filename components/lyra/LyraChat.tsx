...
interface Message {
  ...
  generatedImage?: string; // base64 data URL from image_gen
}
...
function parseMessageContent(raw: string): { text: string; generatedImage?: string } {
  const imgMatch = raw.match(/__IMG__([\s\S]+?)__IMG__/);
  if (imgMatch) {
    const text = raw.replace(/__IMG__[\s\S]+?__IMG__/, "").trim();
    return { text, generatedImage: imgMatch[1] };
  }
  return { text: raw };
}

function ToolCallCard({ json }: { json: string }) {
  try {
...
function MessageContent({ content, generatedImage }: { content: string; generatedImage?: string }) {
  // Split on tool call JSON blocks
  const parts = content.split(/(\{"tool":"[^}]+"[^}]*\})/g);
  return (
    <div>
      {parts.map((part, i) => {
        if (part.startsWith('{"tool":')) {
          return <ToolCallCard key={i} json={part} />;
        }
        if (part.trim()) {
  ...
  async function send() {
    ...
    try {
      ...
      while (true) {
        ...
        const { text: displayText, generatedImage } = parseMessageContent(raw);
        setMessages((prev) => {
          const updated = [...prev];
          updated[updated.length - 1] = { role: "assistant", content: displayText, generatedImage };
          return updated;
        });
      }
            ...
            <div key={i} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
              ...
              <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed max-w-[80%] ${
                ...
                {msg.content === "" && loading && i === messages.length - 1 ? (
                  ...
                  <MessageContent content={msg.content} generatedImage={msg.generatedImage} />
                )}
              </div>
            </div>
          ))}
